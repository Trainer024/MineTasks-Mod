...
public class NoteTaskScreen extends Screen {
    ...
    protected void init() {
        ...
        editTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Edit"), b -> openTaskEditor())
                .dimensions(px + 58, fy1, 44, bh).build());

        deleteTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Del"), b -> deleteSelectedTask())
                .dimensions(px + 104, fy1, 40, bh).build());

        markDoneBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Done"), b -> toggleTaskDone())
                .dimensions(px + 146, fy1, 44, bh).build());

        craftBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Recipes"), b -> openJeiRecipes())
                .dimensions(px + 192, fy1, 48, bh).build());

        usesBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Uses"), b -> openJeiUses())
                .dimensions(px + 242, fy1, 44, bh).build());

        copyBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Copy"), b -> copySelectedTask())
                .dimensions(px + 288, fy1, 44, bh).build());

        dupTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Duplicate"), b -> duplicateTask())
                .dimensions(px + 4, fy2, 72, bh).build());

        resetTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Reset"), b -> resetTaskProgress())
                .dimensions(px + 78, fy2, 48, bh).build());

        pinTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Pin"), b -> togglePin())
                .dimensions(px + 128, fy2, 40, bh).build());

        upTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("↑"), b -> moveTask(-1))
                .dimensions(px + 170, fy2, 28, bh).build());

        downTaskBtn = addDrawableChild(ButtonWidget.builder(Text.literal("↓"), b -> moveTask(1))
                .dimensions(px + 200, fy2, 28, bh).build());

        sortBtn = addDrawableChild(ButtonWidget.builder(Text.literal(sortLabel()), b -> cycleSort())
                .dimensions(px + 230, fy2, 58, bh).build());

        hideDoneBtn = addDrawableChild(toggleBtn(px + 292, fy2, 64, bh, "Hide done", hideCompleted, v -> {
            hideCompleted = v;
            ModConfig.get().hideCompletedInMenu = v;
            ModConfig.save();
        }));

        clearDoneBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Clear done"), b -> clearCompletedTasks())
        ...
        editNoteBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Edit"), b -> openNoteEditor())
                .dimensions(px + 94, fy1, 48, bh).build());

        deleteNoteBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Delete"), b -> deleteSelectedNote())
                .dimensions(px + 144, fy1, 56, bh).build());

        pinNoteBtn = addDrawableChild(ButtonWidget.builder(Text.literal("Pin note"), b -> toggleNotePin())
    ...
    private void processPendingItem(String itemId) {
        // Creates a default new task with the incoming item
        this.client.setScreen(new TaskEditorScreen(this, new ItemTask("New Task", itemId, 1, false)));
    }

    // ... [Rest of your helper methods: sortLabel, cycleSort, toggleBtn, rebuildButtons, etc.] ...
    // Note: ensure your helper methods like rebuildButtons() and render() remain below this point.
    ...
    private List<Integer> visibleTaskIndices() {
        List<Task> tasks = sortedTasksView();
        List<Integer> indices = new ArrayList<>();
        for (int i = 0; i < tasks.size(); i++) {
            Task t = tasks.get(i);
            int realIndex = SaveData.getTasks().indexOf(t);
            if (realIndex >= 0 && matchesTask(t)) indices.add(realIndex);
        }
        return indices;
    }
    ...
    private List<Task> sortedTasksView() {
        ...
        if (mode == 1) {
            copy.sort(Comparator.comparing(t -> t.name.toLowerCase()));
        } else if (mode == 2) {
            copy.sort(Comparator.comparing((Task t) -> !t.pinned)
                    .thenComparing(t -> -t.getProgressFraction())
    ...
    private List<Integer> visibleNoteIndices() {
        List<Note> notes = sortedNotesView();
    ...
    public void render(DrawContext ctx, int mx, int my, float delta) {
        ...
        int tabW = 78;
        boolean tabTasksHover = mx >= px + 6 && mx < px + 6 + tabW && my >= py + 24 && my < py + 36;
        boolean tabNotesHover = mx >= px + 6 + tabW + 4 && mx < px + 6 + tabW * 2 + 4 && my >= py + 24 && my < py + 36;
        UiTheme.drawTab(ctx, textRenderer, "Tasks", px + 6, py + 24, tabW, 14, currentTab == 0, tabTasksHover);
        UiTheme.drawTab(ctx, textRenderer, "Notes", px + 6 + tabW + 4, py + 24, tabW, 14, currentTab == 1, tabNotesHover);

        ctx.fill(px + 4, py + LIST_TOP - 2, px + PW - 4, py + LIST_TOP - 1, UiTheme.BORDER_DIM);
        ctx.fill(px + 4, py + LIST_TOP + LIST_H, px + PW - 4, py + LIST_TOP + LIST_H + 1, UiTheme.BORDER_DIM);

        if (currentTab == 0) renderTaskList(ctx, mx, my, px, py);
        else renderNoteList(ctx, mx, my, px, py);

        if (!tooltip.isEmpty()) {
            UiTheme.drawTooltip(ctx, textRenderer, tooltip, mx, my);
        }
    ...
    private void renderTaskList(DrawContext ctx, int mx, int my, int px, int py) {
        ...
        if (tasks.isEmpty()) {
            ctx.drawText(textRenderer, "No tasks — click '+ Add' or use Quick Add keybind", lx + 4, ly + 10, UiTheme.TEXT_MUTED, false);
            return;
        }
        if (visible.isEmpty()) {
    ...
    private void renderNoteList(DrawContext ctx, int mx, int my, int px, int py) {
        ...
        if (notes.isEmpty()) {
            ctx.drawText(textRenderer, "No notes — click '+ New note'", lx + 4, ly + 10, UiTheme.TEXT_MUTED, false);
            return;
        }
        if (visible.isEmpty()) {
        ...
        for (int row = startRow; row < endRow; row++) {
            ...
            if (!note.body.isEmpty()) {
                ...
                ctx.drawText(textRenderer, trim(preview, lw - 8), lx + 4, iy + 15, UiTheme.TEXT_MUTED, false);
            }
        }
        ctx.disableScissor();
    }
    ...
    public boolean keyPressed(KeyInput input) {
        if (input.key() == GLFW.GLFW_KEY_DELETE) {
            if (currentTab == 0) deleteSelectedTask();
            else deleteSelectedNote();
            return true;
        }
        if (input.key() == GLFW.GLFW_KEY_ENTER) {
            if (currentTab == 0) openTaskEditor();
            else openNoteEditor();
            return true;
        }
